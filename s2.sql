 ...
 * ,row_number()over ( order by amount desc ) as 'ROW NUMBER',
 ...
 select department,amount,
first_value(amount)over(partition by department order by amount desc)as 'first_amount',
last_value(amount)over(partition by department order by amount desc)as last_amount
 desc rows between unbounded preceding and unbounded following)as 
from sales; 
